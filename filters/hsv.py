...
def warm(frame):
    ...
    result = _hsv_adjust(frame, h_shift=-5, s_scale=1.15, v_scale=1.05)
    b, g, r = cv2.split(result)
    r = np.clip(r.astype(np.float32) * 1.12, 0, 255).astype(np.uint8)
    b = np.clip(b.astype(np.float32) * 0.88, 0, 255).astype(np.uint8)
    return cv2.merge([b, g, r])

def cool(frame):
    """Cool tone: shift hue toward blue/cyan, reduce reds."""
    result = _hsv_adjust(frame, h_shift=10, s_scale=0.9, v_scale=1.0)
    b, g, r = cv2.split(result)
...
def fade(frame):
    ...
    result = _hsv_adjust(frame, s_scale=0.45, v_scale=0.95)
